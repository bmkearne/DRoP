...
# Feb. 26, 2012
# Bradley Kearney
# Processes pdb files for DRoP analysis on the webserver.
#*********************************************************************************
...
def run():
    ...
    if(root[-1:] != '/'):
        root += '/'
    url = '''REDACTED''''running.php?job=%d&status=200'%int(id)
    print url
	...
	url='''REDACTED''''.php?job=%d&status=667'%int(id)
	sys.exit(1)
    ...
    if (a.failC1()):
        ...
        url='''REDACTED''''.php?job=%d&status=222'%int(id)
        raw_return=urllib.urlopen(url).read()
        sys.exit(1)
    a.phase1()
    print "Exiting Phase"
