...
# Feb. 26, 2012
# Bradley Kearney
# Main DRoP analysis program for web interface.
#*********************************************************************************
...
class Analyzer:
    ...
    def __init__(self, pdb_filenames, root, id, err=1):
        ...
        self.master_path = '%d/master.html'%self.id
        self.orgmaster_path = '%d/org.html'%self.id
    ...
    def newgroup22(self, max_err):
        ...
        file = open('%d/water_clusters_pass_%d.txt'%(self.id,self.magi), 'w')
    ...
    def newgrouporg2(self, max_err):
        ...
        if self.magi2==0:
            self.org_table=[]
            self.org_table_post=[]
        file = open('%d/org_clusters_pass_%d.txt'%(self.id,self.magi), 'w')
    ...
    def viewgraph(self):
        ...
        plt.savefig("%d/watercluster.png"%self.id,dpi=300,format='png')
	autolabel(rects1)
        autolabel(rects2)
        plt.savefig("%d/watercluster_labeled.png"%self.id,dpi=300,format='png')
	self.color()
	...
	self.org_table.reverse()
        ...
        def autolabel(rects):
            for rect in rects:
                ...
                plt.text(rect.get_x()+rect.get_width()/2., 1.05*height, '%d'%int(height),
                        ha='center', va='bottom')
        plt.savefig("%d/organiccluster.png"%self.id,dpi=300,format='png')
        autolabel(rects1)
	autolabel(rects2)
	plt.savefig("%d/organiccluster_labeled.png"%self.id,dpi=300,format='png')
    ...
    def color(self):
        ...
        print "in waters"
        self.createlist()
        self.pdbs.append(PDB("%d/clusters.pdb"%self.id))
        for pdb in self.pdbs:
            pdb.blankcolor()
        for w in self.waters:
...
def run():
    ...
    if(root[-1:] != '/'):
        root += '/'
    start_err = '1.0'
    if(not os.path.exists('%d'%int(id))):
        os.makedirs('%d'%int(id))
        os.makedirs('%d/color'%int(id))
    url = '''REDACTED''''.php?job=%d&status=400'%int(id)
    ...
    a.phase3()
    a.viewgraph()
    url = '''REDATCTED''''.php?job=%d&status=777'%int(id)
    raw_return=urllib.urlopen(url).read()
    os.chdir('%d'%int(id))
    filenames= os.listdir(os.getcwd())
